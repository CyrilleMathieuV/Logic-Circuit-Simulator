...
class CPUAssemblerEditorMenu extends ToolHandlers {

    public constructor(editor: LogicEditor) {
        super(editor)
    }

    public CPUAssemblerEditor = this.editor.html.CPUAssemblerEditor

    private showContextMenu(menuData: MenuData | undefined, e: MouseEvent | TouchEvent) {
        const hideMenu = () => {
            if (this.CPUAssemblerEditor !== null) {
                this.CPUAssemblerEditor.classList.remove('show-menu')
                this.CPUAssemblerEditor.innerHTML = ""
                this.CPUAssemblerEditor = null
            }
        }
        ...
        if (menuData !== undefined) {
            ...
            this.CPUAssemblerEditor = mainContextMenu
    ...
    handleDragStart = (e: MouseEvent) => {
        this.CPUAssemblerEditor.style.opacity = "0.4"
    }

    handleDragEnd = (e: MouseEvent, item: HTMLElement) => {
        item.style.opacity = "1"
        //for (subitem:  HTMLElement, item : HTMLElement) {
        //   subitem.classList.remove('over')
        //}
    }
    ...
    handleDragEnter = (e: MouseEvent) => {
        //this.CPUAssemblerEditor.add("over");
    }

    handleDragLeave = (e: MouseEvent) => {
        //this.CPUAssemblerEditor.remove("over");
    }

    for (item: HTMLElement, CPUAssemblerEditor: HTMLElement) {
        ...
        /*
        item.addEventListener("dragend", this.handleDragEnd = (e: MouseEvent, item) => {
            item.style.
        })
        */
