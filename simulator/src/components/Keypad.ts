...
type KeypadBaseRepr = KeypadRepr

export abstract class KeypadBase<
    TRepr extends KeypadBaseRepr,
...
export type KeypadRepr = Repr<typeof KeypadDef>
export type KeypadParams = ResolvedParams<typeof KeypadDef>


export class Keypad extends KeypadBase<KeypadBaseRepr> {

    public readonly numBits: number
    private _isPushButton: boolean
    private _isConstant: boolean

    public constructor(parent: DrawableParent, params: KeypadParams, saved?: KeypadBaseRepr) {
        super(parent, KeypadDef.with(params), saved)

        this.numBits = params.numBits

        this._isPushButton = saved?.isPushButton ?? KeypadDef.aults.isPushButton
        this._isConstant = saved?.isConstant ?? KeypadDef.aults.isConstant
    }

    public toJSON() {
        return {
            ...this.toJSONBase(),
            bits: this.numBits === KeypadDef.aults.bits ? undefined : this.numBits,
            val: this.contentRepr(),
            isPushButton: (this._isPushButton !== KeypadDef.aults.isPushButton) ? this._isPushButton : undefined,
            isConstant: (this._isConstant !== KeypadDef.aults.isConstant) ? this._isConstant : undefined,
        }
    }
    ...
    public override makeTooltip() {
        const s = S.Components.Keypad.tooltip
    ...
    protected override makeComponentSpecificContextMenuItems(): MenuItems {
        const s = S.Components.Keypad.contextMenu
